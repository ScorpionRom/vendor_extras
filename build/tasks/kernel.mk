...
ifneq (,$(findstring gki,$(KERNEL_DEFCONFIG)))
    ifeq ($(shell [ -d $(KERNEL_SRC)/scripts/gki ] && \
        ...
        cmd := export $(CORE_MAKE_FLAGS) ARCH=$(KERNEL_ARCH) $(KERNEL_CROSS_COMPILE) KERN_OUT=$(KERNEL_OUT) MAKE_PATH=$(shell dirname $(KERNEL_MAKE_CMD))/ TARGET_BUILD_VARIANT=user && $(TARGET_KERNEL_SOURCE)/scripts/gki/generate_defconfig.sh $(KERNEL_DEFCONFIG)
        _x := $(shell $(cmd))
    endif
endif
...
define build-image-kernel-modules-du
...
$(TARGET_PREBUILT_INT_KERNEL): $(KERNEL_CONFIG) $(DEPMOD) $(DTC)
	...
	$(hide) if grep -q '=m' $(KERNEL_CONFIG); then \
			...
			($(call build-image-kernel-modules-du,$$modules,$(KERNEL_MODULES_OUT),$(KERNEL_MODULE_MOUNTPOINT)/,$(KERNEL_DEPMOD_STAGING_DIR),$(BOARD_VENDOR_KERNEL_MODULES_LOAD))); \
			$(if $(BOOT_KERNEL_MODULES),\
				...
				[ $$? -ne 0 ] && exit 1; \
				($(call build-image-kernel-modules-du,$$vendor_boot_modules,$(TARGET_VENDOR_RAMDISK_OUT),/,$(KERNEL_VENDOR_RAMDISK_DEPMOD_STAGING_DIR),$(BOARD_VENDOR_RAMDISK_KERNEL_MODULES_LOAD))); \
			) \
		fi
...
$(BOARD_PREBUILT_DTBOIMAGE):
	echo -e ${CL_GRN}"Building DTBO image"${CL_RST}
