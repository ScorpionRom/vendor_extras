...
func init() {
	android.RegisterModuleType("scorpion_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = scorpionExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = scorpionExpandVariables(ctx, dep_file)
	...
	cmd := scorpionExpandVariables(ctx, String(g.properties.Cmd))
